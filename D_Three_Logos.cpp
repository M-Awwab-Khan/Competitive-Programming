...
void solve() {
   ...
   vpii logos = {{x1, y1}, {x2, y2}, {x3, y3}};

   
   int area = (x1 * y1) + (x2 * y2) + (x3 * y3);
   double sideLength = sqrt(area);
   if (ceil(sideLength) != floor(sideLength)) {
    print(-1);
    debug("area");
    return;
}
...
rep(rotate_mask, 0, (1 << 3)) {
    vs grid(length, string(length, 'Z'));
    debug(grid);
    int numPlaced = 0;
    rep(i, 0, length) {
        rep(j, 0, length) {
            if (grid[i][j] != 'Z') continue;
            if (numPlaced == 3) goto outer;

            int w = logos[numPlaced].ff;
            int h = logos[numPlaced].ss;

            if (rotate_mask & (1 << numPlaced)) swap(w, h);

            rep(r, i, i + h) {
                rep(c, j, j + w) {
                    if (r >= length || c >= length || grid[r][c] != 'Z') goto outer;
                    grid[r][c] = 'A' + numPlaced;
                }
            }
            ++numPlaced;
