...
int main() {
    ...
    ll ans = 0;
    fnr(i, n) {
        ll x = lower_bound(all(b), a[i]) - b.begin();
        ll y = upper_bound(all(b), a[i]) - b.begin();
        if (x == 0) ans = max(ans, b[x] - a[i]);
        else if (y == m) ans = max(ans, a[i] - b[y - 1]);
        else ans = max(ans, min(a[i] - b[y - 1], b[x] - a[i]));
    }
    print(ans);
    return 0;
}
