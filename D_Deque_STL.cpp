...
void solve() {
    // it is a classic sliding window maximum problem, i will try to do it using set or ordered_set (TLE)
    int n, k; cin >> n >> k;
    vi a(n); rep(i, 0, n) cin >> a[i];

    // ordered_set<pii> os;
    // rep(i, 0, k) os.insert({a[i], i});

    // cout << os.find_by_order(k-1)->ff << " ";
    // rep(i, k, n) {
    //     os.insert({a[i], i});
    //     os.erase({a[i-k], i-k});
    ...
    rep(i, 0, n) {
        ...
        // pop old elements from the front
        while (!dq.empty() && dq.front() <= i-k) dq.pop_front();

        // pop elements that can't be max from the back
