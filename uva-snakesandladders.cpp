...
int main() {
    ...
    while(t--) {
        ...
        fnr(i, c) {
            ...
            if (jumps.find(playerPositions[currentPlayer]) != jumps.end()) {
                playerPositions[currentPlayer] = jumps[playerPositions[currentPlayer]];
            }
            if (playerPositions[currentPlayer] >= 100) {
                gameFinished = true;
                maxScore = playerPositions[currentPlayer];
            }
            currentPlayer++;
            if (currentPlayer > a) currentPlayer = 1;
        }
        for (auto u: playerPositions) {
            cout << "Position of player " << u.f << " is " << u.s << ".\n";
        }
    }
    return 0;
}
