...
void solve() {
    ...
    rep(mask, 0, (1 << m)) {
        ...
        rep(i, 0, m) {
            // check if that ac is on
            if (!(mask & (1 << i))) continue;
            auto [ai, bi, pi, mi] = acs[i];
            cost += mi;

            //  diff array trick (cooling effect of [ai, bi] increases by pi)
            cooling[ai] += pi;
            cooling[bi + 1] -= pi;
        ...
        // if all the cows are satisfied then ans = min(ans, cost)
        rep(i, 0, n) {
            auto [si, ti, ci] = cows[i];
            rep(j, si, ti + 1) {
                if (cooling[j] < ci) goto outer;
            }
        }
