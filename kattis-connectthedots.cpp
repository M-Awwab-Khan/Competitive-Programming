...
int main() {
    ...
    while (1) {
        ...
        fnr(i, n) {
            fnr(j, m) {
                if (image[i][j] != '.') {
                    points[image[i][j]] = make_pair(i, j);
                }
            }
        }
        vc sequence;
        ...
        fnr(i, sequence.size() - 1) {
            if (points.find(sequence[i+1]) != points.end()) {
                pii start = points[sequence[i]];
                pii end = points[sequence[i+1]];
                if (start.f == end.f) {
                    if (start.s < end.s) {
                        cf(j, start.s, end.s) {
                            if (image[start.f][j] == '|') {
                                image[start.f][j] = '+';
                            } else if (image[start.f][j] == '.') {
                                image[start.f][j] = '-';
                            }
                        }
                    }
                    ...
                    if (start.f < end.f) {
                        cf(j, start.f, end.f) {
                            if (image[j][start.s] == '-') {
                                image[j][start.s] = '+';
                            } else if (image[j][start.s] == '.') {
                                image[j][start.s] = '|';
                            }
                        }
                    }
                    else {
                        for (int j = start.f; j >= end.f; --j) {
                            if (image[j][start.s] == '-') {
                                image[j][start.s] = '+';
                            } else if (image[j][start.s] == '.') {
                                image[j][start.s] = '|';
