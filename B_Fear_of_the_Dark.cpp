...
bool isInside(ld w, pii &c, pii p) {
    ll dx = c.ff - p.ff;
    ll dy = c.ss - p.ss;
    return dx*dx + dy*dy <= w*w; 
}

bool isIlluminated(ld w, pii &a, pii &b, pii &p) {
    // check if the path from (0, 0) to (px, py) is illuminated or not
    pii home = {0, 0};
    
    // If home is inside circle a and work is inside circle a, path is illuminated
    if (isInside(w, a, home) && isInside(w, a, p)) return true;
    
    // If home is inside circle b and work is inside circle b, path is illuminated
    if (isInside(w, b, home) && isInside(w, b, p)) return true;
    
    // If home is in circle a, work is in circle b, and circles touch/overlap
    if (isInside(w, a, home) && isInside(w, b, p)) {
        ll dx = a.ff - b.ff;
        ll dy = a.ss - b.ss;
        return (dx*dx + dy*dy) <= (4 * w * w);
    }
    
    // If home is in circle b, work is in circle a, and circles touch/overlap
    if (isInside(w, b, home) && isInside(w, a, p)) {
        ll dx = a.ff - b.ff;
        ll dy = a.ss - b.ss;
        return (dx*dx + dy*dy) <= (4 * w * w);
    }
    
    // Otherwise, no valid illuminated path exists
    return false;
}
