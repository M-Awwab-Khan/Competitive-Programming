...
bool canRestXDays(ll x, ll n, ll P, ll l, ll t) {
    // work from x + 1 th day till n
    // how many labs unlocked on day x + 1

    ll p = 0;
    
    // take all the lessons from now on
    p += (n - x) * l;

    // labs unlocked
    ll labs = min((ll)floor((n-1) / 7.0) + 1, 2*(n - x));
