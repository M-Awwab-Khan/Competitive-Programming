...
void solve() {
    ...
    rep(i, 0, n) {
        cin >> s;
        rep(j, 0, d) if (s[j] == '1') survives.insert(j);
    }

    si dies;
    rep(i, 0, d) {
        if (!survives.count(i)) dies.insert(i);
    }
    // survival positions pushed in the set.
    // if the incoming DNA contains all the present genes in the set, then that survives.
    ...
    rep(i, 0, q) {
        ...
        rep(j, 0, d) {
            if (s[j] == '1') {
                cur.insert(j);
                if (survives.count(j)) sv++;
                if (dies.count(j)) di++;
            }
        }
        if (di == dies.size()) cout << "dies" << endl;
        else if (di == 0) cout << "survives" << endl;
