...
void solve() {
   ...
   vector<pair<char, int>> a(all(freq));
    ...
    s = "";
    rep(i, 0, a.size()) s += string(a[i].second, a[i].first);
    ...
    int l = 0;
    int r = l + ws - 1;
    auto maxPalindromeLen = [&](int left, int right) -> int {
        umap<char, int> mp;
        rep(i, left, right + 1) mp[s[i]]++;
        int ans = 0;
        bool odd = false;
        for (auto &[k, v]: mp) {
           if (v % 2== 0) ans += v;
           else {
            if (!odd){ans += v; odd = true;}
            else ans += v- 1;
           }
        }
        // debug(ans);
    ...
    while (r < n) {
        if (n - (r + 1) < ws) {
            fans = min(fans, maxPalindromeLen(l, n - 1));
        } else fans = min(fans, maxPalindromeLen(l, r));
