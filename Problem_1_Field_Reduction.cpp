...
void solve() {
    ...
    auto cmpX = [](const Cow& A, const Cow& B){ return A.x < B.x; };
    auto cmpY = [](const Cow& A, const Cow& B){ return A.y < B.y; };
    vector<Cow> sx = a, sy = a;
    sort(sx.begin(), sx.end(), cmpX);
    sort(sy.begin(), sy.end(), cmpY);

    si candSet;
    int k = min(3, n); // up to 3 from each side
    for (int i = 0; i < k; ++i) {
        candSet.insert(sx[i].id);                 // leftmost
        candSet.insert(sx[n-1-i].id);            // rightmost
        candSet.insert(sy[i].id);                 // bottom
        candSet.insert(sy[n-1-i].id);            // top
    }
    ...
    for (int mask = 0; mask < totalMasks; ++mask) {
        ...
        for (const auto& c : a) {
            int p = pos[c.id];
            if (p != -1 && (mask & (1 << p))) continue; // removed candidate
            // Non-candidates are never removed
