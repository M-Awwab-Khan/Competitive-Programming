...
int main() {
    ...
    int n; cin>>n;
    vll q(n); fnr(i, n) cin>>q[i]; // ingredients we have
    vll a(n); fnr(i, n) cin>>a[i]; // required ingredients for dish A
    vll b(n); fnr(i, n) cin>>b[i]; // required ingredients for dish B

    ll ans = 0;
    fnr(x, *max_element(all(q)) + 1) {
        // assume you can make x dishes of type A
        ll y = LLONG_MAX;
        fnr(i, n) {
            if (q[i] - x * a[i] < 0) { // you cannot make x dishes of type A
                y = -LLONG_MAX;
                break;
            }
            else if (b[i] > 0) {

                y = min(y, (q[i] - x * a[i]) / b[i]); // if you can then how many dishes of type B can you make?
            }
        }
        ans = max(ans, x + y);
    }
