...
ll manhattan_distance(ll cx, ll cy, ll ex, ll ey) {
...
int main() {
    ...
    string s; cin >> s;
    ll dist = manhattan_distance(sx, sy, ex, ey);
    // cout << "initial distance " << dist << endl;
    int seconds = 0;
    fnr(i, t) {
            // cout << "distance " << dist << " " << s[i] << endl;

        if (s[i] == 'N' && manhattan_distance(sx, sy + 1, ex, ey) < dist) {
            dist = manhattan_distance(sx, sy + 1, ex, ey);
            ++sy;
        }

        else if (s[i] == 'E' && manhattan_distance(sx + 1, sy, ex, ey) < dist) {
            dist = manhattan_distance(sx + 1, sy, ex, ey);
            ++sx;
        }
        else if (s[i] == 'S' && manhattan_distance(sx, sy - 1, ex, ey) < dist) {
            dist = manhattan_distance(sx, sy - 1, ex, ey);
            --sy;
        }
        else if (s[i] == 'W' && manhattan_distance(sx - 1, sy, ex, ey) < dist) {
            dist = manhattan_distance(sx - 1, sy, ex, ey);
            --sx;
        }
        ++seconds;
        if (sx == ex && sy == ey) goto found;
    }
    print(-1);
    return 0;
    found: print(seconds);
