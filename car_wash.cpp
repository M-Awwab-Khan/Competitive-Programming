...
void solve() {
    ...
    cin >> n;
    ll e1, e2; cin >> e1 >> e2;
    vll a1(n); rep(i, 0, n) cin >> a1[i];
    vll a2(n); rep(i, 0, n) cin >> a2[i];

    vll s12(n); rep(i, 1, n) cin >> s12[i];
    vll s21(n); rep(i, 1, n) cin >> s21[i];
    ...
    rep(i, 1, n) {
        dp1[i] = min(dp1[i-1] + a1[i], dp2[i-1] + s21[i] + a1[i]);
        dp2[i] = min(dp2[i-1] + a2[i], dp1[i-1] + s12[i] + a2[i]);
    }
