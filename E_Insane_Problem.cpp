...
void solve() {
    ...
    auto smallestX = [&](ll k) {
        ll left = l1, right = r1;
        // sabse chota x in [l1, r1] jo  (x*k) [l2, r2] mein lie kare
        ll a = 0;
        while (left <= right) {
            ll mid = (left + right) / 2;
            if (mid *k >= l2) {
    ...
    auto largestX = [&](ll k) {
        ll left = l1, right = r1;
        // sabase bara x in [l1, r1] such that (x*k) [l2, r2] mein lie kare
    ...
    rep(i, 0, 32) {
        ...
        ll smallest = smallestX(base);
        ll largest = largestX(base);
