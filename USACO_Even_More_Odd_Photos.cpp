...
void solve() {
    ...
    int ans = min(odds, evens);
    odds -= ans;
    evens -= ans;
    ans *= 2;

    // now any one of them is 0
    if (odds == 0) {
        print(ans + 1); 
        return;
    } else {
        // add groups of 3 odds (each contributes 2 to answer)
        ans += (odds / 3) * 2;
        int rem = odds % 3;
        
        if (rem == 1) {
            // one odd left, need to remove one pair to make room for it
            ans = max(0, ans - 1);
        } else if (rem == 2) {
            // two odds left, can make one more even position
            ans += 1;
        }
