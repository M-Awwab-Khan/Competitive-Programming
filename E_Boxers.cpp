...
void solve() {
    ...
    for (int i = 0; i < n; i++) {
        // Try to use weight-1 first if possible
        if (a[i] > 1 && !used.count(a[i] - 1)) {
            used.insert(a[i] - 1);
        }
        // Try to use original weight if possible
        else if (!used.count(a[i])) {
            used.insert(a[i]);
        }
        // Try to use weight+1 if possible
        else if (!used.count(a[i] + 1)) {
            used.insert(a[i] + 1);
        }
    }
