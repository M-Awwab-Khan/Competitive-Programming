...
ull modmul(ull a, ull b, ull M) {return ((__uint128_t)a*b) % M;}
ll modexp(ll a,ll b,ll m) { ll res=1; while (b>0) { if (b&1) res=modmul(res, a, m); a=modmul(a, a, m); b>>=1; } return res; }
void yes() { cout<<"YES\n"; }
void no() { cout<<"NO\n"; }
...
bool isPrime(ull n) {
    ...
    for (ull a : A) {
        if (a >= n) break;
        ull p = modexp(a, d, n), i = s;
        if (p == 1 || p == n - 1) continue;
        while (i-- > 1) {p = modmul(p, p, n); if (p == n-1) break;}
        if (p != n - 1) return 0;
    } return 1;
}
