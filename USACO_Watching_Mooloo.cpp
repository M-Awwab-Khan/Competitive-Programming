...
void solve() {
    ...
    // the key is to group as many of them as possible
    // this can be done efficiently using sliding window technique

    int l = 0;
    ll ans = 0;
    rep(r, 0, n) {

        // either include this day within previous window, or start a new window
        if (r > 0) {
            ll delta = (a[r] - a[r-1]);
            if (delta > (1 + k)) {
                // start a new window
