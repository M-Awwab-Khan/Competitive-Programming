...
void solve() {
    ...
    // A bulb j will be ON at the end if and only if j has an odd number of divisors
    // A number has an odd number of divisors if and only if it's a perfect square
    // So we need exactly k perfect squares <= n
    ...
    while (l <= r) {
        ll x = l + (r - l) / 2;
        ll sq = (ll) sqrtl(x);
        // Handle precision issues by checking sq-1, sq, and sq+1
        if ((sq + 1) * (sq + 1) <= x) sq++;
        while (sq * sq > x) sq--;
