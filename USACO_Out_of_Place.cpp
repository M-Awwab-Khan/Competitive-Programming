...
void solve() {
    ...
    // now you need to figure out the closest position where you can place bessie

    bool ahead = false;
    bool behind = false;
    
    if (bessie > 0) ahead = (a[bessie-1] > a[bessie]);
    if (bessie < n) behind = (a[bessie + 1] < a[bessie]);
    // debug(ahead, behind);
    
    // a set to only count the distinct values.
    si s;
    if (ahead) {
        int i = bessie - 1;
        while (i > 0 && a[i] > a[bessie]) {s.insert(a[i]); --i;}
    } else {
        int i = bessie + 1;
        while (i < n && a[i] < a[bessie]) {s.insert(a[i]); ++i;}
    }
    int swaps = len(s);
    print(swaps);
}
