...
void solve() {
    int n; cin >> n;
    mii freq;
    vi a(n); rep(i, 0, n) {cin >> a[i]; freq[a[i]]++;}

    int band = a[0]; // the bitwise and
    rep(i, 1, n) band &= a[i];

    // formula, how many ways can i select two elements (equal to band) from all the elements equal to band
    // multiplied by the number of permutattions of the rest of elements.

    debug(band, freq);
    if (freq[band] < 2) print(0);
    else {
        debug(fact[freq[band]], invfact[2], invfact[freq[band] - 2]);
        ll npr = (ll)fact[freq[band]] * invfact[freq[band] - 2] % MOD;
        debug(npr);
        ll ans = npr * fact[n-2] % MOD;
        print(ans);
    }
}
