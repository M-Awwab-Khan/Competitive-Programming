...
int main() {
    ...
    string counter_moves;
    int n = moves.length();
    for (int i = 0; i < n; i++) {
        if (i + 2 < n) {
            string combo = moves.substr(i, 3);
            if (combo == "RBL" || combo == "BLR" || combo == "LRB" || combo == "LBR" || combo == "BRL" || combo == "RLB") {
                counter_moves += 'C';
                i += 2;
                continue;
            }
        }
        if (moves[i] == 'R') counter_moves += 'S';
        else if (moves[i] == 'B') counter_moves += 'K';
        else if (moves[i] == 'L') counter_moves += 'H';
    }
    print(counter_moves);
