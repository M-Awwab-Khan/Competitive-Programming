...
void solve() {
    ...
    auto countSumLessThanX = [&](int X) -> ll {
        ll pairs = 0;
        // kitne aise pairs hain jinka sum <= X hai
        // fix i then binary search the maximum j such that a[i] + a[j] <= X;
        rep(i, 0, n) {
            int y = X - a[i];
            auto it = upper_bound(a.begin() + i + 1, a.end(), y);
            pairs += (it - (a.begin() + i + 1));
        }
        return pairs;
    };

    print(countSumLessThanX(r) - countSumLessThanX(l - 1));
