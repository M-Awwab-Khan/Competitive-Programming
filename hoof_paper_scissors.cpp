...
int main() {
    ...
    ll n;
    cin >> n;
    vll hoof(n + 1, 0), paper(n + 1, 0), scissors(n + 1, 0);
    cf(i, 1, n) {
        hoof[i] += hoof[i - 1];
        paper[i] += paper[i - 1];
    ...
    ll max_wins = 0;
    fnr(i, n+1) {
        ll before_wins = max(hoof[i], max(paper[i], scissors[i]));
        ll after_wins = max(hoof[n] - hoof[i], max(paper[n] - paper[i], scissors[n] - scissors[i]));
