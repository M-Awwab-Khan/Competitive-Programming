...
void solve() {
    int n; cin >> n;
    vs cows = {"Beatrice", "Sue", "Belinda", "Bessie", "Betsy", "Blue", "Bella", "Buttercup"};
    sort(all(cows));

    vector<pair<string, string>> besides(n);
    string cow;
    rep(i, 0, n) {
        cin >> cow;
        besides[i].ff = cow;
    ...
    do {
        ...
        rep(i, 0, n) {
            auto [c1, c2] = besides[i];
            // check if this constraint is satisfied
            bool satisfied = false;
            rep(j, 0, 8) {
                if (cows[j] == c1) {
                    if (j + 1 < 8) satisfied = satisfied || cows[j + 1] == c2;
                    if (j - 1 >= 0) satisfied = satisfied || cows[j - 1] == c2; 
                }
                if (satisfied) break;
            }
            if (!satisfied) goto outer;
        }
