...
void solve() {
    ...
    for (int r = 1, l = 0; r <= n; r++) {
        // Move left pointer if current segment is not valid (not divisible)
        while (l < r - 1 && length[r - 1] < r - l) l++;
        
        // Move left pointer if sum exceeds k
        while (l < r && pref[r] - pref[l] > k) l++;
        
        // Update answer if valid segment found
        if (pref[r] - pref[l] <= k) {
            ans = max(ans, (ll)(r - l));
        }
    }
