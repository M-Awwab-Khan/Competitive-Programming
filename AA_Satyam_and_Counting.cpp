...
void solve() {
    ...
    rep(i, 0, n) {
        if (y[i] == 0) {
            // if there is a point right above it
            if (points.count({x[i], 1})) {
                ans += yzeros - 1;
                ans += yones - 1;
            }


            // if the point is on top right or left
            bool a = (points.count({x[i] + 1, 1}) && points.count({x[i] + 2, 0}));
    ...
    rep(i, 0, n) {
        if (y[i] == 1) {
            bool a = (points.count({x[i] + 1, 0}) && points.count({x[i] + 2, 1}));
            ans += a;
        }
    }
    print(ans);
