...
void solve() {
    ...
    while (c != 0 && n != 0) {
        ...
        // if n >= c there always exist an answer (pigeonhole principle)
        if (c > n) {print("no sweets"); return;}
        mii remainders_seen;
        remainders_seen.insert({0, -1});
        int csummodc = 0;
        ll start = -1;
        ll end = -1;
    
        fnr(i, n) {
            csummodc = (csummodc + a[i]) % c;
            if (remainders_seen.count(csummodc)) {
                start = remainders_seen[csummodc] + 1;
                end = i;
                break;
            } else {
                remainders_seen[csummodc] = i;
            }
        }
        if (start != -1) {
            cf(i, start + 1, end + 1) prints(i);
            cout << endl;
        } else print("no sweets");
        cin >> c >> n;
    }
}
