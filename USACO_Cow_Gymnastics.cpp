...
void solve() {
    int k, n; cin >> k >> n;
    vvi a(k, vi(n));
    rep(i, 0, k) rep(j, 0, n) cin >> a[i][j];

    // for each pair of numbers from 1...N
    int ans = 0;
    rep(i, 1, n + 1) {
        rep(j, i + 1, n + 1) {
            int diff;
            rep(x, 0, n) {
                rep(y, x + 1, n) {
                    if ((a[0][x] == i && a[0][y] == j) || (a[0][x] == j && a[0][y] == i)) {
                        diff = a[0][x] - a[0][y];
                        break;
                    }
                }
            }
            // this difference should remain consistent
            bool consistent = true;
            rep(z, 1, k) {
                rep(x, 0, n) {
                    rep(y, x + 1, n) {
                        if ((a[z][x] == i && a[z][y] == j)|| (a[z][x] == j && a[z][y] == i)) {
                            if (a[z][x] - a[z][y] != diff) {
                                consistent = false;
                            }
                        }
