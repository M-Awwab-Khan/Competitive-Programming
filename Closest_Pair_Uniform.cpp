...
ld distance(const pair<ld, ld>& a, const pair<ld, ld>& b) {
...
void solve() {
    while (true) {
        ...
        fnr(i, n) cin >> points[i].ff >> points[i].ss;
        multimap<ld, ld> active; // sorted with y-coordinate

        int j = 0;
        ld minDist = LDBL_MAX;
        pair<pair<ld, ld>, pair<ld, ld>> bestPair;
        sort(all(points));
        fnr(i, n) {
            // remove far points
            pair<ld, ld> current = points[i];
            while (points[j].ff < current.ff - minDist) {
                active.erase(active.find(points[j].ss));
                ++j;
            }

            // find points in range [curr - minDist, curr + minDist] according to y-coordinate
            auto lower = active.lower_bound(current.ss - minDist);
            auto upper = active.upper_bound(current.ss + minDist);

            for (auto it = lower; it != upper; ++it) {
                pair<ld, ld> p = {it->second, it->first};
                ld dist = distance(current, p);
                if (dist < minDist) {
                    minDist = dist;
                    bestPair = {{p.ss, p.ff}, current};
                }
            }

            active.insert({current.ss, current.ff}); // insert point into multimap
        }
        cout << setprecision(2) << fixed;
        cout << bestPair.ff.ss << " " << bestPair.ff.ff << " " << bestPair.ss.ff << " " << bestPair.ss.ss << endl;
