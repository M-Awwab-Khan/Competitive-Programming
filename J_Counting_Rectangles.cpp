...
void solve() {
    int n, q; cin >> n >> q;
    vpii a(n); rep(i, 0, n) cin >> a[i].ff >> a[i].ss;

    vvll pref(1002, vll(1002, 0));
    //preprocessing

    rep(i, 0, n) {
        pref[a[i].ff][a[i].ss] += 1LL * a[i].ff * a[i].ss;
    }
    ...
    while (q--) {
        cin >> h1 >> w1 >> h2 >> w2;
        print(pref[h2-1][w2-1] - pref[h1][w2-1] - pref[h2-1][w1] + pref[h1][w1]);
    }
}
