...
void constructSequence(int n, ll sum, int l, int r, vi &seq) {
    ll minSum = n * l;
    ll maxSum = n * r;

    if (sum < minSum || sum > maxSum) return;

    ll remaining = sum - minSum;

    fnr(i, n) {
        ll add;
        if (remaining <= 0) break;
        if (remaining + l > r) add = r - l;
        else add = remaining;
...
int main() {
    ...
    ll sk, sall;
    int n,k,l,r; cin >> n >> k >> l >> r >> sall >> sk;
    ...
    sort(right);
    vi left(n-k, l);
    constructSequence(fl, sl, l, right[0], left);
    fora(left) prints(u);
    fora(right) prints(u);
    cout << endl;
