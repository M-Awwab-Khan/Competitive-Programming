...
void solve() {
    ll H, N; cin >> H >> N;
    vll a(N), b(N); fnr(i, N) cin >> a[i] >> b[i];

    // dp[i] is the minimum magic points required to kill the monster of health i
    // dp[i] = min(dp[i], dp[i - a[j]] + b[j]) for all j such that i - a[j] >= 0
