...
int main() {
    ...
    int n, m; cin >> n >> m;
    vll a(n); fnr(i, n) cin >> a[i];
    if (n > m) {   // by pigeonhole this is true the two modulos will be same hence remainder 0
        yes();
        return 0;
    }
    ...
    fnr(i, n) {
        ...
        dp = new_dp;
        dp[a[i] % m] = true;  // use this number alone in the sequence only after processing this iteration
    }
