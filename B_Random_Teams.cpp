...
int main() {
    ...
    ll ans = 0;
    if (m == 1) {
        cout << n*(n-1)/2 << " " << n*(n-1)/2 << endl; return 0;
    }
    ll x = n/m  // number of participants in each team
    , y = n%m; // number of teams with one extra participant
    ll a = x*(x-1)/2*(m-y) + (x+1)*(x)/2*y; // number of pairs in each team

    // for maximum number of pairs
    ans = n - ( m - 1);
    ans = ans*(ans-1)/2;
