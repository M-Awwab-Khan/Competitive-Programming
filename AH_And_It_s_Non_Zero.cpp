...
void solve() {
    ...
    int ans = INT_MAX;
    rep(x, 0, msb + 1) {
        debug(x, pref[r][x], pref[l-1][x]);
        ans = min(ans, pref[r][x] - pref[l-1][x]);
    }
    print(ans);
}
...
int main() {
    ...
    // pref[i][x] is the number of zeros in bit x from 1 to i

    rep(i, 1, MAXN + 1) {
        rep(x, 0, 32) {
            pref[i][x] = pref[i-1][x] + !((1 << x) & i);
        }
    }
    int t; cin >> t; while(t--)
    solve();
