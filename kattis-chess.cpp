...
int main() {
    ...
    while(t--) {
        ...
        // if they are at opposite color squares
        ...
        // if the are at the same diagonal
        if ((srow - scol) == (erow - ecol) || (srow + scol) == (erow + ecol)) {
            cout << "1 " << a << " " << src << " " << b << " " << dest << endl;
            continue;
        }


        // move your bishop in both diagonals until you land on a diagonal on which the destination square lies
        vector<vi> dirs = {{1, 1}, {1, -1}, {-1, 1}, {-1, -1}};
        bool found = false;
        for (auto dir : dirs) {
            if (found) break;
            int row = srow, col = scol;
            while (row >= 0 && row < 8 && col >= 0 && col < 8) {
                if ((row - col) == (erow - ecol) || (row + col) == (erow + ecol)) {
                    cout << "2 " << a << " " << src << " " << (char)('A' + col) << " " << 8 - row << " " << b << " " << dest << endl;
                    found = true;
                    break;
                }
                row += dir[0];
