...
void solve() {
    ...
    umap<int, pii> firstAndLast;
    vi a(n); rep(i, 0, n) {
        cin >> a[i]; freq[a[i]]++;
        if (!firstAndLast.count(a[i])) {
            firstAndLast[a[i]].first = i;
            firstAndLast[a[i]].second = -1;
        } else {
            firstAndLast[a[i]].second = i;
        }
        // debug(firstAndLast);
    }
