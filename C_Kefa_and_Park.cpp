...
int ans = 0;
void dfs(int root, vi &cats, vvi &adj, int m, int c, vector<bool> &visited) {
    visited[root] = true;
    ...
    for (int u : adj[root]) {
        if (!visited[u]) {
            isLeaf = false;
            dfs(u, cats, adj, m, c, visited);
        }
    }

    if (isLeaf && root != 1) ++ans;
}
int main() {
    cin.tie(0); cout.tie(0);
    int n, m; cin >> n >> m;
    vi cats(n); fnr(i, n) cin >> cats[i];
