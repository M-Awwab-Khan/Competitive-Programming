...
int solve(vi &a, int i, int sum, int ones) {
    if (sum == 0) return ones;
    if (i < 0) return INT_MAX;

    if (a[i] > sum) {
        return solve(a, i-1, sum, ones);
    }
    return min(solve(a, i - 1, sum, ones), solve(a, i, sum-a[i], ones+(a[i]==1)));
}
int main() {
