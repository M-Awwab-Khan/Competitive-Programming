...
template <typename... A> constexpr auto maxx(const A&... a){using T=std::common_type_t<A...>;return (std::max)({T(a)...});}
template <typename... A> constexpr auto minn(const A&... a){using T=std::common_type_t<A...>;return (std::min)({T(a)...});}
...
struct Rect {
    ...
    pii intersection(const Rect &r) {
        int x = min(x2, r.x2) - max(x1, r.x1);
        int y = min(y2, r.y2) - max(y1, r.y1);
        if (x > 0 && y > 0) {
            return {x, y};
        }
        return {0, 0};
...
void solve() {
    ...
    // If it's impossible in both dimensions simultaneously, answer is -1
    if (w + curW > W && h + curH > H) {
        print(-1);
        return;
    }
    ...
    if (w + curW <= W) {
        // Space needed to the left or right
        double moveLeft  = max(0, w - x1);        // shift left so left free space becomes >= w
        double moveRight = max(0, w - (W - x2));  // shift right so right free space becomes >= w
        ans = min(ans, min(moveLeft, moveRight));
    }

    // Try fitting vertically
    if (h + curH <= H) {
        double moveDown = max(0, h - y1);
        double moveUp   = max(0, h - (H - y2));
        ans = min(ans, min(moveDown, moveUp));
    }
