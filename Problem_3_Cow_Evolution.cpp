...
bool isOverlapping(si &a, si &b) {
    si res;
    set_intersection(all(a), all(b), inserter(res, res.begin()));
    if (res.size() == a.size() || res.size() == b.size() || res.size() == 0) return 0;
    return 1;
}
