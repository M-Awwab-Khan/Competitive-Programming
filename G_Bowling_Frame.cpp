...
void solve() {
    ...
    // try to make maximum number of complete rows with w white pins
    ll wrs = 0;
    ll l = 0, r = 1e9;
    ...
    // ans complete rows bn saktin from w
    ll lastRowW = w - (1LL * ans * (ans + 1))/2;
    ll need = (ans + 1) - lastRowW;

        debug(ans);
    if (b >= need) {
        b -= need;
        ll n;
        if ((lastRowW + need) == (ans + 1)) {
            n = ans + 2;
            ++ans;
        } else {
            n = ans + 1;
        ...
        while (l <= r) {
            ll mid = (l + r) /2;
            // pins req to build midth row
            ll x = (((1LL * mid * (mid + 1) )/ 2)) - (((1LL * startRow * (startRow + 1))/2));
            if (x <= b) {
                ans = mid;
                l = mid + 1;
            } else r = mid - 1;
