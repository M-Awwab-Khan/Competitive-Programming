...
int main() {
    ...
    map<char, int> wrong_submissions;
    map<char, int> correct_submissions;
    while (true) {
        int n;
        string prob, status;
        cin >> n;
        ...
        cin >> prob >> status;
        if (status == "right") {
            if (correct_submissions.find(prob[0]) == correct_submissions.end()) {
                correct_submissions[prob[0]] = n;
            }
        } else {
            wrong_submissions[prob[0]]++;
        }
    }
    ll penalties = 0;
    fora(correct_submissions) {
        penalties += correct_submissions[u.f] + wrong_submissions[u.f] * 20;
    }
    cout << correct_submissions.size() << " " << penalties << endl;
    return 0;
}
