...
void solve() {
    ...
    // dp[i][j] is the number of ways to distribute j candies to first i children.
    vvll dp(n+1, vll(k+1, 0));

    dp[0][0] = 1;

    cf(i,1, n) {
        vll pref(k + 2, 0);  // we need up to k+1 for prefix sums
        pref[0] = dp[i-1][0];
        ...
        cf(j, 0, k) {
            int left = j - a[i-1] - 1;
            if (left >= 0) {
                dp[i][j] = (pref[j] - pref[left] + MOD) % MOD;
            } else {
                dp[i][j] = pref[j];
            }
        }
