...
void solve() {
    ...
    int k_coins_used = min(ak, k_coins);

    m -= k*k_coins_used;

    // If remaining amount can be covered by regular coins
    if (a1 >= m || m <= 0) {
        print(ans);
    } else {
        // Use all regular coins first
        m = max(0, m - a1);
        
        // Calculate how many fancy coins needed
        // We need to use k-value fancy coins optimally
        int need_fancy_k = m / k;
        int need_fancy_1 = m % k;

        int needa1 = (k - (m % k) + k) % k;
        if (a1 >= needa1) {
            m += needa1;
            ans += m / k;
            print(ans);
            ...
            // If it's better to use a k-value fancy coin instead of k 1-value fancy coins
            if (need_fancy_k > 0) {
                ans += need_fancy_k;
            }
            print(ans);
