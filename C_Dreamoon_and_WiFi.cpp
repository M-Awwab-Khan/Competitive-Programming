...
double correct = 0;
double wrong = 0;
void recurse(string &s1, string &s2, int pluses, int minuses, int pos) {
    if (pos == s2.size()) {
        map<char, int> mp;
        fnr(i, s2.size()) ++mp[s2[i]];
        if (mp['+'] == pluses && mp['-'] == minuses) ++correct;
        else ++wrong;
        return;
    }
...
int main() {
    ...
    map<char, int> mp2;
    fnr(i, s1.size()) ++mp2[s1[i]];

    recurse(s1, s2, mp2['+'], mp2['-'], 0);
