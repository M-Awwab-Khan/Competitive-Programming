...
void solve() {
    ...
    auto feasible = [&](ld midR, ld &left, ld &right) {
        ld L = -1e30L, R = 1e30L;
        rep(i, 0, n) {
            if (midR < t[i]) return false;
            ld Li = x[i] - (midR - t[i]);
            ld Ri = x[i] + (midR - t[i]);
    ...
    rep(i, 0, 200) {
        ld mid = (l + r) / 2.0;
        ld left, right;
        if (feasible(mid, left, right)) r = mid;
        else l = mid;
    }

    ld leftx0; ld rightx0;
    feasible(r, leftx0, rightx0);
    ld ans = ( leftx0 + rightx0 ) / 2.0L;
