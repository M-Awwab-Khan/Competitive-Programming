...
struct SegTree {
    int n;
    vector<T> tree;
    function<T(const T&, const T&, const T&)> merge;
    T identity;
    int levels;

    SegTree(int n, function<T(const T&, const T&, const T&)> mergeFunc, T identity)
        : n(n), merge(mergeFunc), identity(identity), levels(ceil(log2(n))) {
        tree.assign(4 * n, identity);
    }

    void build(const vector<T> &arr, int v, int tl, int tr, int level) {
        if (tl == tr) {
            tree[v] = arr[tl];
            return;
        }
        int tm = (tl + tr) / 2;
        build(arr, v*2, tl, tm, level-1);
        build(arr, v*2+1, tm+1, tr, level-1);
        tree[v] = merge(tree[v*2], tree[v*2+1], level);
    }

    void build(const vector<T> &arr) {
        build(arr, 1, 0, n-1, levels);
    }

    void update(int v, int tl, int tr, int pos, T val, int level) {
        if (tl == tr) {
            tree[v] = val;
            return;
        }
        int tm = (tl + tr) / 2;
        if (pos <= tm) update(v*2, tl, tm, pos, val, level-1);
        else update(v*2+1, tm+1, tr, pos, val, level-1);
        tree[v] = merge(tree[v*2], tree[v*2+1], level);
    }

    void update(int pos, T val) {
        update(1, 0, n-1, pos, val, levels);
    }
    ...
    T query(int v, int tl, int tr, int l, int r, int level) {
        ...
        int tm = (tl + tr) / 2;
        return merge(
            query(v*2, tl, tm, l, min(r, tm), level-1),
    ...
    T query(int l, int r) {
        return query(1, 0, n-1, l, r, levels);
    }
};
...
void solve() {
    int n, m; cin >> n >> m;
    vll a((1<<n)); rep(i, 0, (1LL << n)) cin >> a[i];

    SegTree<ll> t((1LL << n), [](const ll &a, const ll &b, const ll &level) { return level % 2 ? (a | b) : (a ^ b);}, 0);
    t.build(a);

    while (m--) {
        int p, b; cin >> p >> b;
        t.update(--p, b);
        print(t.query(0, (1LL<<n)-1));
    }
