...
void solve() {
    ...
    auto orientation = [](ld x1, ld y1, ld x2, ld y2, ld x3, ld y3) -> int {
        ld val = (y2 - y1) * (x3 - x2) - (x2 - x1) * (y3 - y2);
        if (fabs(val) < 1e-9) return 0;  // collinear
        return (val > 0) ? 1 : 2;        // clockwise or counterclockwise
    };
    
    auto onSegment = [](ld x1, ld y1, ld x2, ld y2, ld x3, ld y3) -> bool {
    ...
    // Special Cases
    if (o1 == 0 && onSegment(x1, y1, x2, y2, x3, y3)) { print("YES"); return; }
    if (o2 == 0 && onSegment(x1, y1, x2, y2, x4, y4)) { print("YES"); return; }
    if (o3 == 0 && onSegment(x3, y3, x4, y4, x1, y1)) { print("YES"); return; }
    if (o4 == 0 && onSegment(x3, y3, x4, y4, x2, y2)) { print("YES"); return; }
