...
bool isValidState(string &current, string &given) {
    return current.size() == given.size();
}

vc getCandidates(string &current, string &given, vector<bool> visited) {
    vc candidates;
    sc used;
    fnr(i, given.size()) {
        if (!visited[i] && !used.count(given[i])) {
            candidates.pb(i);
            used.insert(given[i]);
        }
    }
...
void backtrack(string &given, string &current, vector<string> &result, vector<bool> visited) {
    if (isValidState(current, given)) {
        result.pb(current);
        return;
    }
    vc candidates = getCandidates(current, given, visited);
    fora(candidates) {
        visited[u] = true;
        current.pb(given[u]);
        backtrack(given, current, result, visited);
        current.pop_back();
        visited[u] = false;
    }
}
