...
template <typename T, typename... A> constexpr T maxx(const T& a, const A&... b){return (std::max)({a,b...});}
template <typename T, typename... A> constexpr T minn(const T& a, const A&... b){return (std::min)({a,b...});}
...
void solve() {
    int n, b; cin >> n >> b;
    vector<pii> cows(n); rep(i, 0, n) cin >> cows[i].ff >> cows[i].ss;


    si hcandidates;
    si vcandidates;

    rep(i, 0, n) hcandidates.insert(cows[i].ff + 1);
    rep(i, 0, n) vcandidates.insert(cows[i].ss + 1);


    int ans = n;
    for(auto x: hcandidates) {
        for (auto y: vcandidates) {

            int tl = 0, tr = 0, br = 0, bl = 0;
            for (auto [xi, yi]: cows) {
                if (xi < x && yi > y) ++tl;
                else if (xi > x && yi > y) ++tr;
                else if (xi > x && yi < y) ++br;
                else ++bl;
            }

            ans = minn(ans, maxx(tl, tr, br, bl));
        }
    }
    print(ans);
}
