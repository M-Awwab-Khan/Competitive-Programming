...
int main() {
    ...
    // no of descending segments should atmost be 1
    // the element before the segment should be smaller than last element of segment
    // the element after the segment should be larger than first element of segment
    int n; cin >> n;
    vi a(n);
    ...
    if (segments.size() > 1) print("no");
    else if (segments.size() == 0) {
        print("yes");
        print("1 1");
    }
    else {
        int starting = segments[0].f;
        int ending = segments[0].s;
        bool ok1 = false;
        bool ok2 = false;

        if (starting <= 0) ok1 = true;
        else {
            if (a[starting-1] < a[ending]) ok1 = true;
            else ok1 = false;
        }
        if (ending >= n-1) ok2 = true;
        else {
            if (a[ending+1] > a[starting]) ok2 = true;
            else ok2 = false;
        }
