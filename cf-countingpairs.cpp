...
void solve() {
    ...
    while (t--) {
        ...
        for (int i = 0; i < n; ++i) {
            ...
            long long lower_limit = remaining_sum - y;
            long long upper_limit = remaining_sum - x;

            auto lower = lower_bound(a.begin() + i + 1, a.end(), lower_limit);
            auto upper = upper_bound(a.begin() + i + 1, a.end(), upper_limit);
