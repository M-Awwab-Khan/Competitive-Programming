...
int main() {
    ...
    vpii points(n);
    fnr(i, n) cin >> points[i].f >> points[i].s;

    unordered_set<string> killed;
    int ans = 0;
    fnr(i, n) {
        if (!killed.count(to_string(points[i].f) + " " + to_string(points[i].s))) {
            killed.insert(to_string(points[i].f) + " " + to_string(points[i].s));
            fnr(j, n) {
                if (collinear(x0, y0, points[i].f, points[i].s, points[j].f, points[j].s)) {
                    killed.insert(to_string(points[j].f) + " " + to_string(points[j].s));
