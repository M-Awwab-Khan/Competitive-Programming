...
void dfs(int days, int b1, int b2, vi barn1, vi barn2) {
    if (days == 4) {
        res.insert(b1); return;
    }
    ...
    if (days % 2 == 0) {
        rep(i, 0, len(barn1)) {
            ...
            dfs(days + 1, b1 - bucket, b2 + bucket, newb1, newb2);
        }
    }
    // second to first
    else {
...
void solve() {
    vi barn1(10); rep(i, 0, 10) cin >> barn1[i];
    vi barn2(10); rep(i, 0, 10) cin >> barn2[i];
