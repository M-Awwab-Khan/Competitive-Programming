...
void solve() {
    ...
    // either begin or end with B or conssecutive Bs
    zeroGroup = s[0] == 'B' || s[n-1] == 'B';
    rep(i, 0, n) zeroGroup = zeroGroup || ((s[i] == s[i + 1]) && s[i] == 'B');
    ...
    int ans = 0;
    ans += accumulate(all(groups), 0);

    if (!zeroGroup) ans -= groups[0];
