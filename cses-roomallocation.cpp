...
int main() {
    ...
    int n; cin >> n;
    vector<pair<pair<int, int>, ll>> a;  // stores {(arrival, departure), index}
    vi assignment(n); // stores the room number assigned to each guest
    ...
    int last_room = 0;
    priority_queue<pii, vector<pii>, greater<pii>> pq; // stores {departure, room number}
    fnr(i, n) {
        if (pq.empty() || pq.top().f >= a[i].f.f) {
            last_room++;
            assignment[a[i].s] = last_room;
            pq.push({a[i].f.s, last_room});
        } else {
            pii minimum = pq.top();
            pq.pop();
            assignment[a[i].s] = minimum.s;
            pq.push({a[i].f.s, minimum.s});
        }
    }
